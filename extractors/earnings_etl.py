...
MAPPING_FILE = os.path.join(SCRIPT_DIR, "..", "mapping_files", "earnings_line_item_mapping.json")
...
def handle_earnings_files():
    ...
    files = [f for f in os.listdir(DATA_DIR) if f.endswith("_earnings.csv")]
    ...
    print("[INFO] Starting earnings import...")
    for file in tqdm(files, desc="Processing files"):
        path = os.path.join(DATA_DIR, file)
        try:
            df = pd.read_csv(path)
            symbol = file.replace("_earnings.csv", "")
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), desc=f"{file}", leave=False):
                ...
                period_end = row.get("period", None)
            ...
            if valid_rows:
                try:
                    pd.DataFrame(valid_rows).to_sql("earnings", engine, if_exists="append", index=False)
