...
MAPPING_FILE = os.path.join(SCRIPT_DIR, "..", "mapping_files", "cash_flow_line_item_mapping.json")
...
def handle_cash_flow_files():
    ...
    files = [f for f in os.listdir(DATA_DIR) if f.endswith("_cash_flow.csv") or f.endswith("cashflow.csv")]
    ...
    print("[INFO] Starting cash flow import...")
    for file in tqdm(files, desc="Processing files"):
        path = os.path.join(DATA_DIR, file)
        try:
            df = pd.read_csv(path)
            symbol = file.replace("_cash_flow.csv", "").replace("cashflow.csv", "").replace(".csv", "")
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), desc=f"{file}", leave=False):
                ...
                if pd.isna(period_end):
                    skip_reason.append("missing period_end")
                if not period_start:
            ...
            if valid_rows:
                try:
                    pd.DataFrame(valid_rows).to_sql("cash_flow", engine, if_exists="append", index=False)
