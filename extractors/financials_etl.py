...
MAPPING_FILE = os.path.join(SCRIPT_DIR, "..", "mapping_files", "financials_line_item_mapping.json")
...
def handle_financials_files():
    ...
    files = [f for f in os.listdir(DATA_DIR) if f.endswith("_financials.csv")]
    ...
    print("[INFO] Starting financials import...")
    for file in tqdm(files, desc="Processing files"):
        path = os.path.join(DATA_DIR, file)
        try:
            df = pd.read_csv(path)
            symbol = file.replace("_financials.csv", "")
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), desc=f"{file}", leave=False):
                ...
                if pd.isna(period_end):
                    skip_reason.append("missing period")
                if not period_start:
            ...
            if valid_rows:
                try:
                    pd.DataFrame(valid_rows).to_sql("financials", engine, if_exists="append", index=False)
    ...
    if unmapped_items:
        ...
        print("\n[WARNING] All rows were skipped due to missing or invalid fields. Nothing to ingest.")
    else:
        print("\n[INFO] All line_item values successfully mapped. Ready for DB ingestion.")

if __name__ == "__main__":
    handle_financials_files()
